        ...
        <Modal title={orderModel.isCancel ? "Подробная информация о заявке" : "Причина отмены"} setShow={setShow} show={isOpen}>
            {
                orderModel.isCancel ? <>
                    ...
                    <div className="flex gap-[48px] mb-[14px]">
                        ...
                        <p className={`rounded-full py-[6px] px-[8px] text-[10px] bg-[${bgColor}] font-normal text-xs flex items-center justify-center leading-normal text-center text-white`}>{OrderStatusText[el]}</p>
                    </div>
                    ...
                    {
                        order?.orderStatusId === 5 ? <span className="px-[49px] text-[#ad0606] py-[10px]">Отменен</span> : <Button onClick={() => orderModel.toggleCancel()} class='w-[max-content] font-[700] px-[49px] !text-[#ad0606] py-[10px] rounded-[8px] border-[2px] border-solid border-[#ad0606]' children="Отменить" />
                    }
                </> :
                    <>
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex flex-row gap-6 items-center">
                                <Button onClick={() => { orderModel.cancelOrder((id) => orderListModel.changeOrderStatus(id, OrderStatus.Cancelled)); setShow(false); orderModel.toggleCancel() }} children="Отменить" class="bg-[#4A85F6] w-max !py-3 px-6" />
                                <Button onClick={() => orderModel.toggleCancel()} class='w-[max-content] !text-[#4A85F6] font-semibold !py-2 !px-6 rounded-[8px]' children="Назад" />
                            </div>
                        </div>
                    </>
            }
