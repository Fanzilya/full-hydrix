...
type Props = {
    ...
    plantsResult?: PlantsStatsResult,
    maxWidth?: boolean,
}

type PlantsStatsResult = {
    orderCount: number,
...
export const Table = observer((props: Props) => {
    ...
    useEffect(() => {
        table.getPaginationRowModel().rows.slice(1, 2).map(row => {
            row.getVisibleCells().map(cell => {
                console.log(cell.column.getSize() + "px")
            })
        })
    }, [])
            ...
            <table className={`w-full border border-[#EFF4FA] mt-4 ${props.class}`}>
                    ...
                    <div className="overflow-auto max-h-[80vh]">
                        <div className="w-max">
                            ...
                            <tbody className="bg-white">
                                ...
                                {props.plantsResult &&
                                    <tr className="flex w-full cursor-pointer bg-[#4A85F6] font-bold text-white rounded-[0_0_8px_8px]">
                                        <td className="2xl:text-[14px] text-[12px] text-center py-6 px-5 first:pl-16 last:pr-10"
                                            key={Guid.create().toString()}
                                            style={{ flex: "260px" }}
                                        >
                                            ВСЕГО
                                        ...
                                        <td className="2xl:text-[14px] text-[12px] text-center py-6 px-5 first:pl-16 last:pr-10"
                                            ...
                                            style={{ flex: "150px" }}
                                        >
                                            {props.plantsResult.orderCount}
                                        </td>
                                        <td className="2xl:text-[14px] text-[12px] text-center py-6 px-5 first:pl-16 last:pr-10"
                                            key={Guid.create().toString()}
                                            style={{ flex: "150px" }}
                                        >
                                            {props.plantsResult.recycleVolume}
                                        </td>
                                        <td className="2xl:text-[14px] text-[12px] text-center py-6 px-5 first:pl-16 last:pr-10"
                                            key={Guid.create().toString()}
                                            style={{ flex: "150px" }}
                                        >
                                            {props.plantsResult.dailyLimit}
                                        </td>
                                        <td className="2xl:text-[14px] text-[12px] text-center py-6 px-5 first:pl-16 last:pr-10"
                                            key={Guid.create().toString()}
                                            style={{ flex: "150px" }}
                ...
                <div className="w-full pb-5 mt-5">
                    <div className="flex flex-row items-center gap-5 justify-center">
                        ...
                        {/* <span className="text-[#717171] text-[14px] flex items-center">
                            Страница <span className="text-[#222B45]">{table.getState().pagination.pageIndex + 1} из {table.getPageCount()}</span>
                        </span> */}
